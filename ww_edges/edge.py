...
def edge(img : np.array, device : str) -> np.array:
    ...
    np1 = (mask1*255).to(torch.uint8).expand([height, width, 3]).cpu().numpy()

    # common sigma
    ret_std = torch.sqrt(std.sum()).cpu().numpy()
